...
The exporter brokers a narrow Redis read operation for each scraper. Scrapers do not supply the Redis destination or arbitrary Redis commands. Deployment operators choose the destination and key patterns; Redis writers choose the names and data structures the exporter encounters. These are different capabilities. A compromised Redis writer does not automatically obtain exporter process or cloud authority, and a legitimate scrape is not itself an unauthorized database read.
